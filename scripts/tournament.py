...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run a round-robin tournament between Reinforce Tactics bots")
    parser.add_argument("--map", help="Path to single map file (for backward compatibility)")
    parser.add_argument("--maps", nargs="+", help="List of map file paths to use in evaluation")
    parser.add_argument("--map-dir", help="Directory to load all maps from (alternative to listing individual maps)")
    parser.add_argument(
        "--map-pool-mode",
    ...
    parser.add_argument("--models-dir", default="models", help="Path to models directory (default: models/)")
    parser.add_argument(
        "--output-dir", default="tournament_results", help="Path for results and replays (default: tournament_results/)"
    )
    parser.add_argument(
        "--games-per-side",
        type=int,
    ...
    parser.add_argument("--max-turns", type=int, default=500, help="Maximum turns per game (default: 500)")
    parser.add_argument(
        "--test", action="store_true", help="Test mode: add duplicate SimpleBots for testing the tournament system"
    )
    parser.add_argument("--log-conversations", action="store_true", help="Enable LLM conversation logging to JSON files")
    parser.add_argument(
        "--conversation-log-dir", help="Directory for conversation logs (default: output_dir/llm_conversations/)"
    )
    parser.add_argument("--concurrent", type=int, default=1, help="Number of concurrent games (default: 1, sequential)")
    parser.add_argument("--no-llm", action="store_true", help="Skip LLM bot discovery")
    parser.add_argument("--no-models", action="store_true", help="Skip trained model bot discovery")
