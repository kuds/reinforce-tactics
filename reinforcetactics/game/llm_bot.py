...
# System prompt - compact version (saves ~40-60% tokens)
SYSTEM_PROMPT_COMPACT = """Reinforce Tactics: Turn-based strategy. Win by capturing enemy HQ or eliminating all enemies.
...
# System prompt - verbose version (better for weaker models)
SYSTEM_PROMPT_VERBOSE = """You are an expert player of Reinforce Tactics, a turn-based strategy game.
...
- Only ONE unit can occupy any tile
- Each action in your list is executed sequentially
- If enemies are within 2-3 tiles of your HQ, defending it is TOP priority

Respond with ONLY the JSON object below. No extra text before or after."""

# Default to compact mode (can be overridden per-bot)
...
class LLMBot(ABC):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, game_state, player: int = 2, api_key: Optional[str] = None,
        ...
        Args:
            ...
            compact_mode: Use compact token-saving format (default True).
                When True, uses abbreviated keys and compact JSON (saves ~40-60% tokens).
                When False, uses verbose descriptive keys (better for weaker models).
        """
    ...
    def take_turn(self):
        ...
        response_text = None
        system_prompt = SYSTEM_PROMPT_COMPACT if self.compact_mode else SYSTEM_PROMPT_VERBOSE
    ...
    def _serialize_game_state(self) -> Dict[str, Any]:
        ...
        for row in self.game_state.grid.tiles:
            for tile in row:
                if tile.type in ['b', 'h', 't']:
                    ...
                    if self.compact_mode:
                        # Compact: [type, x, y] - income derivable from type
                        building_info = [tile.type, orig_x, orig_y]
                    else:
                        # Verbose: full dict with income
                        building_info = {
                            'type': tile.type,
                            'position': [orig_x, orig_y],
        ...
        # Format legal actions for the LLM (filter out empty action types in compact mode)
        ...
        if self.compact_mode:
            ...
            filtered_legal_actions = formatted_legal_actions
        ...
        if self.compact_mode:
            ...
            return {
                ...
                'enemy_buildings': enemy_buildings,
                'neutral_buildings': neutral_buildings,
                'legal_actions': filtered_legal_actions
            }
    ...
    def _format_legal_actions(self, legal_actions: Dict[str, List[Any]],
        ...
        # Create unit actions
        create_key = 'create' if self.compact_mode else 'create_unit'
        for action in legal_actions['create_unit']:
            orig_x, orig_y = self.game_state.padded_to_original_coords(
                action['x'], action['y']
            )
            if self.compact_mode:
                # Compact: [type, x, y, cost]
                formatted[create_key].append([
        ...
        for action in legal_actions['move']:
            if action['unit'] in unit_id_map:
                ...
                if self.compact_mode:
                    # Compact: [unit_id, to_x, to_y]
                    formatted['move'].append([
                        unit_id_map[action['unit']], to_x, to_y
                    ])
                else:
                    # Verbose: full dict with from position
                    formatted['move'].append({
...
Use only legal actions from state."""
            ...
            # Verbose response format with indented JSON
