...
class TestTournamentSystem:
    ...
    def test_tournament_runner_initialization(self):
        ...
        config = TournamentConfig(
            maps=[MapConfig(path="maps/1v1/beginner.csv", max_turns=500)], output_dir="/tmp/test_tournament", games_per_side=2
        )
        _runner = TournamentRunner(config)
    ...
    def test_tournament_runner_with_logging_parameters(self):
        ...
        _runner1 = TournamentRunner(config1)
    ...
    def test_tournament_results_structure(self):
        ...
        results.add_game_result(
            GameResult(
                game_id=1, bot1_name="Bot1", bot2_name="Bot2", winner=1, winner_name="Bot1", turns=10, map_name="test.csv"
            )
        )
        results.add_game_result(
            GameResult(
                game_id=2, bot1_name="Bot1", bot2_name="Bot2", winner=1, winner_name="Bot1", turns=15, map_name="test.csv"
            )
        )
...
class TestModelBotActionTranslation:
    ...
    def test_create_unit_action(self):
        ...
        buildings_p2 = [
            (tile.x, tile.y) for row in game_state.grid.tiles for tile in row if tile.type in ["b", "h"] and tile.player == 2
        ]
...
class TestBotFactory:
    ...
    def test_bot_factory_create_bot_with_model_path(self):
        ...
        with patch("reinforcetactics.game.model_bot.ModelBot") as mock_modelbot:
            ...
            mock_modelbot.assert_called_once_with(game_state, player=2, model_path="/path/to/model.zip")
...
class TestMultiMapTournament:
    ...
    def test_tournament_config_from_dict(self):
        ...
        config_dict = {
            "maps": [{"path": "maps/1v1/beginner.csv", "max_turns": 500}],
    ...
    def test_tournament_per_map_stats_tracking(self):
        ...
        from reinforcetactics.tournament import GameResult, TournamentResults
        ...
        results.add_game_result(
            GameResult(
                game_id=1, bot1_name="Bot1", bot2_name="Bot2", winner=1, winner_name="Bot1", turns=10, map_name="map1.csv"
            )
        )
        results.add_game_result(
            GameResult(
                game_id=2, bot1_name="Bot1", bot2_name="Bot2", winner=2, winner_name="Bot2", turns=15, map_name="map2.csv"
            )
        )
        results.add_game_result(
            GameResult(
                game_id=3, bot1_name="Bot1", bot2_name="Bot2", winner=1, winner_name="Bot1", turns=20, map_name="map1.csv"
            )
        )
    ...
    def test_generate_results_with_elo(self):
        ...
        results.add_game_result(
            GameResult(
                game_id=2, bot1_name="Bot1", bot2_name="Bot2", winner=1, winner_name="Bot1", turns=15, map_name="map1.csv"
            )
        )
