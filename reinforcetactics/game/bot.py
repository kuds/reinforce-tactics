...
class AdvancedBot(MediumBot):
    """Advanced AI bot extending MediumBot with map analysis and special abilities."""
    ...
    def take_turn(self):
        ...
        # Phase 2: Use MediumBot's superior purchase strategy
        self.purchase_units()

        # Phase 3: Enhanced unit actions with special abilities
    ...
    def act_with_unit_enhanced(self, unit):
        """Enhanced version of MediumBot's act_with_unit with special abilities."""
        # Check if already seizing a structure
        tile = self.game_state.grid.get_tile(unit.x, unit.y)
        if (tile.is_capturable() and tile.player != self.bot_player and
                tile.health < tile.max_health):
        ...
        # Try special abilities first (enhancement over MediumBot)
        if self.try_use_special_ability(unit):
            return

        # Try ranged attacks (enhancement for Archers/Mages)
        if unit.type in ['A', 'M'] and self.try_ranged_attack(unit):
            return

        # Fall back to MediumBot's proven strategy
        # Use the standard act_with_unit from MediumBot
        super().act_with_unit(unit)
